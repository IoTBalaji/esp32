...
String SendHTML(bool isNotFound)
{
	...
	htmlContent += "<head>";
		...
		htmlContent += "<link href='https://cdn.jsdelivr.net/gh/IoTBalaji/esp32@v0.0.2/index.css' rel='stylesheet' crossorigin='anonymous'>";
	htmlContent += "</head>";
	htmlContent += "<body>";
	...
	if (isNotFound) {
		...
		htmlContent += "</div>";
		htmlContent += "<script src='https://cdn.jsdelivr.net/gh/IoTBalaji/esp32@v0.0.2/index.js' crossorigin='anonymous'></script>";
	}
	htmlContent += "</div>";
	htmlContent += "</html>";
	return htmlContent;
